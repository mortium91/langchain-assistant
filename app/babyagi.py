...
async def process_task_telegram(objective: str, base_url: str, chat_id: str, client=None):
    first_task = {
        "task_id": 1,
        ...
        while True:
            if task_list:
                ...
                print("\033[95m\033[1m" +
                      "\n*****TASK LIST*****\n"+"\033[0m\033[0m")
                await client.get(f"{base_url}/sendMessage?chat_id={chat_id}&text=*****TASK LIST*****\n")
                temp = ""
                for t in task_list:
                    tsk = str(t['task_id'])+": "+t['task_name']
                    print(tsk)
                    temp = temp + tsk + "\n"
                await client.get(f"{base_url}/sendMessage?chat_id={chat_id}&text={temp}")

                # Step 1: Pull the first task
                task = task_list.popleft()
                print("\033[92m\033[1m" +
                      "\n*****NEXT TASK*****\n"+"\033[0m\033[0m")
                await client.get(f"{base_url}/sendMessage?chat_id={chat_id}&text=\n*****NEXT TASK*****\n")
                next_tsk = str(task['task_id'])+": "+task['task_name']
                print(next_tsk)
                await client.get(f"{base_url}/sendMessage?chat_id={chat_id}&text={next_tsk}")
                ...
                await client.get(f"{base_url}/sendMessage?chat_id={chat_id}&text=\n*****TASK RESULT*****\n")
                print(result)
                await client.get(f"{base_url}/sendMessage?chat_id={chat_id}&text={result}")
